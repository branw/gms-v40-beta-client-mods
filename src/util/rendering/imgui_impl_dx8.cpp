...
void ImGui_ImplDX8_RenderDrawData(ImDrawData* draw_data)
{
    ...
    //IDirect3DStateBlock8* d3d8_state_block = NULL;
    //if (bd->pd3dDevice->CreateStateBlock(D3DSBT_ALL, &d3d8_state_block) < 0)
    //    return;
    //if (d3d8_state_block->Capture() < 0)
    //{
...
struct ImGui_ImplDX8_ViewportData
{
    ...
    ImGui_ImplDX8_ViewportData()  { SwapChain = nullptr; ZeroMemory(&d3dpp, sizeof(D3DPRESENT_PARAMETERS)); }
    ~ImGui_ImplDX8_ViewportData() { IM_ASSERT(SwapChain == nullptr); }
};
...
static void ImGui_ImplDX8_DestroyWindow(ImGuiViewport* viewport)
{
    ...
    if (ImGui_ImplDX8_ViewportData* vd = (ImGui_ImplDX8_ViewportData*)viewport->RendererUserData)
    {
        if (vd->SwapChain)
...
{
    ImGui_ImplDX8_Data* bd = ImGui_ImplDX8_GetBackendData();
    auto *vd = (ImGui_ImplDX8_ViewportData*)viewport->RendererUserData;
    if (vd->SwapChain)
    {
        vd->SwapChain->Release();
        vd->SwapChain = nullptr;
